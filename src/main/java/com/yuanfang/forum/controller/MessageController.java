...
//        Integer.valueOf("abc");
        ...
        if(conversationList != null){
            for(Message message : conversationList){
                ...
                map.put("unreadCount", messageService.getUnreadLetterCount(user.getId(), message.getConversationId()));
        ...
        //查询未读消息数量
        int letterUnreadCount = messageService.getUnreadLetterCount(user.getId(), null);  //总的未读消息数量
        model.addAttribute("letterUnreadCount", letterUnreadCount);
        int noticeUnreadCount = messageService.getNoticeUnreadCount(user.getId(), null);  //通知的未读数量
    ...
    @ResponseBody  //发送异步请求，向页面回传JSON数据
...
//        Integer.valueOf("abc");
        ...
        message.setFromId(hostHolder.getUser().getId());  //发信人的id
        message.setToId(target.getId()); // 收信人的id
    ...
    public String getNoticeList(Model model){
        ...
        //查询评论类的通知
        Message message = messageService.getLatestNotice(user.getId(), TOPIE_COMMENT);
        ...
        if(message != null){
            ...
            int count = messageService.getNoticeCount(user.getId(), TOPIE_COMMENT);
            messageVO.put("count",count);

            int unread = messageService.getNoticeUnreadCount(user.getId(), TOPIE_COMMENT);
            messageVO.put("unread",unread);
        }
        model.addAttribute("commentNotice",messageVO);

        //查询点赞类的通知
        message = messageService.getLatestNotice(user.getId(), TOPIE_LIKE);
        ...
        if(message != null){
            ...
            int count = messageService.getNoticeCount(user.getId(), TOPIE_LIKE);
            messageVO.put("count",count);

            int unread = messageService.getNoticeUnreadCount(user.getId(), TOPIE_LIKE);
            messageVO.put("unread",unread);
        }
        model.addAttribute("likeNotice",messageVO);

        //查询关注类的通知
        message = messageService.getLatestNotice(user.getId(), TOPIE_FOLLOW);
        messageVO = new HashMap<>();
        messageVO.put("message",message);
        if(message != null){
            ...
            int count = messageService.getNoticeCount(user.getId(), TOPIE_FOLLOW);
            messageVO.put("count",count);

            int unread = messageService.getNoticeUnreadCount(user.getId(), TOPIE_FOLLOW);
            messageVO.put("unread",unread);
        }
        model.addAttribute("followNotice",messageVO);

        //查询未读消息的数量（包括私信未读消息的总数量和系统通知未读的总数量）
